...
export default function DeleteSponsorModal({ open, onClose, onConfirm, sponsor, deleting }) {
  if (!open) return null;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="w-full max-w-sm bg-[var(--bg-card)] border border-[var(--border)] rounded-[var(--radius-xl-value)] shadow-[var(--shadow-lg-value)] p-6 text-center">
        ...
        <p className="text-sm text-[var(--text-secondary)] mb-6">
          ...
          <span className="font-medium text-[var(--text-primary)]">{sponsor?.name}</span>?
          This will also remove its logo. This action cannot be undone.
        </p>
