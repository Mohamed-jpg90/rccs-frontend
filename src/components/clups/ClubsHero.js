...
export default function ClubsHero({ stats, search, onSearchChange }) {
  return (
    <section className="relative overflow-hidden bg-[var(--bg-surface)] border-b border-[var(--border)]">
      ...
      <div className="relative max-w-5xl mx-auto px-6 md:px-10 pt-20 pb-14 text-center">
        ...
        <p className="text-[var(--text-secondary)] max-w-lg mx-auto mb-10 leading-relaxed">
          Every club has its own rhythm — pick one, show up, and the rest
          follows.
        </p>

        <div className="grid grid-cols-2 gap-3 md:gap-5 max-w-sm mx-auto mb-10">
          {[
            { label: "Active Clubs", value: stats.total },
            { label: "Open Spots", value: stats.openSpots },
          ].map((s) => (
            <div
              key={s.label}
